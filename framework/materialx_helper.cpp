...
namespace vkb
{
	...
	void  load_document() 
	{
		MaterialX::DocumentPtr mtlxDocument = nullptr;
		mtlxDocument = MaterialX::createDocument();


		MaterialX::NodePtr shaderNode = mtlxDocument->addNode("standard_surface", "", "surfaceshader");
		MaterialX::NodePtr materialNode = mtlxDocument->addMaterialNode("", shaderNode);
		materialNode->setName("Plastic");
		materialNode->setInputValue("base", 1.0f);
		materialNode->setInputValue("base_color", MaterialX::Color3(0.10470402f, 0.24188282f, 0.81800002f));
		materialNode->setInputValue("specular_roughness", 0.32467532157897949f);

        std::cout << mtlxDocument->asString();
