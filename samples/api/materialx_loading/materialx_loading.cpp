...
#include "materialx_loading.h"

#include "scene_graph/components/sub_mesh.h"

MaterialXLoading::MaterialXLoading()
{
	title = "MaterialX loading and rendering";
}
...
void MaterialXLoading::request_gpu_features(vkb::PhysicalDevice &gpu)
{
...
void MaterialXLoading::build_command_buffers()
{
...
void MaterialXLoading::create_attachment(VkFormat format, VkImageUsageFlagBits usage, FrameBufferAttachment *attachment)
{
...
void MaterialXLoading::prepare_offscreen_buffer()
{
	{
...
void MaterialXLoading::load_assets()
{
	// Models
	...
	// Load MaterialXLoading cube map
	textures.envmap = load_texture_cubemap("textures/uffizi_rgba16f_cube.ktx");
}

void MaterialXLoading::setup_descriptor_pool()
{
...
void MaterialXLoading::setup_descriptor_set_layout()
{
...
void MaterialXLoading::setup_descriptor_sets()
{
...
void MaterialXLoading::prepare_pipelines()
{
	...
	shader_stages[0]                  = load_shader("materialx_loading/composition.vert", VK_SHADER_STAGE_VERTEX_BIT);
	shader_stages[1]                  = load_shader("materialx_loading/composition.frag", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	// Bloom pass
	shader_stages[0]                           = load_shader("materialx_loading/bloom.vert", VK_SHADER_STAGE_VERTEX_BIT);
	shader_stages[1]                           = load_shader("materialx_loading/bloom.frag", VK_SHADER_STAGE_FRAGMENT_BIT);
	...
	shader_stages[0] = load_shader("materialx_loading/gbuffer.vert", VK_SHADER_STAGE_VERTEX_BIT);
	shader_stages[1] = load_shader("materialx_loading/gbuffer.frag", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void MaterialXLoading::prepare_uniform_buffers()
{
...
void MaterialXLoading::update_uniform_buffers()
{
...
void MaterialXLoading::update_params()
{
	uniform_buffers.params->convert_and_update(ubo_params);
}

void MaterialXLoading::draw()
{
...
bool MaterialXLoading::prepare(vkb::Platform &platform)
{
...
void MaterialXLoading::render(float delta_time)
{
	if (!prepared)
		return;
	draw();
...
void MaterialXLoading::on_update_ui_overlay(vkb::Drawer &drawer)
{
...
bool MaterialXLoading::resize(const uint32_t width, const uint32_t height)
{
...
std::unique_ptr<vkb::Application> create_materialx_loading()
{
	return std::make_unique<MaterialXLoading>();
}
